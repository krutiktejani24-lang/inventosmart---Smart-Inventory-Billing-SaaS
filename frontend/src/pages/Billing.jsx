  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="border border-slate-200 rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    {['Item Name','HSN','Qty','Price','Disc%','GST%',''].map(h => (
                  ...
                  {form.items.map((item, i) => (
                    <tr key={i} className="border-b border-slate-100 last:border-0">
                      ...
                      <td className="px-2 py-1.5 w-8">
                        {form.items.length > 1 &&
                          <button onClick={() => removeItem(i)} className="text-slate-300 hover:text-red-400"><X size={14}/></button>}
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <style>{`
        ...
        .inp { width:100%; border:1px solid #e2e8f0; border-radius:8px; padding:8px 12px; font-size:13px; color:#0f172a; outline:none; }
        .inp:focus { border-color:#818cf8; }
        .inp-sm { width:100%; border:1px solid #e2e8f0; border-radius:6px; padding:5px 8px; font-size:12px; color:#0f172a; outline:none; }
        .inp-sm:focus { border-color:#818cf8; }
      `}</style>
    </div>
  );
}
...
function InvoiceDetailModal({ invoice, onClose, onRefresh }) {
  ...
  const handlePDF = () => window.open(`${import.meta.env.VITE_API_URL}/invoices/${invoice.id}/pdf`, '_blank');
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 sticky top-0 bg-white z-10">
          ...
          <div className="flex gap-2">
            <button onClick={handlePDF} className="flex items-center gap-1 text-xs bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-lg text-slate-600">
              <Download size={13}/> PDF
            </button>
            <button onClick={handleEmail} disabled={sending} className="flex items-center gap-1 text-xs bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-lg text-indigo-600">
              ...
              {(invoice.items||[]).map((item, i) => (
                <tr key={i} className="border-t border-slate-100">
                  <td className="px-3 py-2">
                    ...
                    {item.hsn_code && <p className="text-xs text-slate-400">HSN: {item.hsn_code}</p>}
                  </td>
                      ...
                      <tr key={inv.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => setViewing(inv)} className="p-1.5 rounded-lg hover:bg-indigo-50 text-indigo-400 hover:text-indigo-600">
                              <Eye size={15}/>
                            </button>
